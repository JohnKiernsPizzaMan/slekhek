...
void u::Render()
{
	...
	if (ImGui::Button("aimbot")) { tab = 0; }
	ImGui::SameLine();
	if (ImGui::Button("visuals")) { tab = 1; }
	ImGui::SameLine();
	if (ImGui::Button("misc")) { tab = 2; }
	ImGui::SameLine();
	if (ImGui::Button("skins")) { tab = 3; }
	ImGui::SameLine();
	if (ImGui::Button("config")) { tab = 4; }
	switch (tab)
	{
	case 1:
	{
		if (ImGui::Button("Switch menu")) {
			otherMenu = true;
		}
		break;
	}
